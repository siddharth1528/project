...
def build_pandas_prompt(query, df_NCR=None, df_FCD=None, chat_history=None, source=None):
    # Determine which DF is relevant
    if source == "ncr":
        df_desc = f"- df_NCR: Contains Non-Conformance Reports (NCRs) with columns: {list(df_NCR.columns)}"
    elif source == "fcd":
        df_desc = f"- df_FCD: Contains Field Change Documents (FCDs) with columns: {list(df_FCD.columns)}"
    else:
        df_desc = (
            f"- df_NCR: Contains Non-Conformance Reports (NCRs) with columns: {list(df_NCR.columns)}\n"
            f"- df_FCD: Contains Field Change Documents (FCDs) with columns: {list(df_FCD.columns)}"
        )
...
def answer_query(query: str):
    ...
    if intent in ["count", "table"]:
        ...
        show_both = not show_ncr and not show_fcd  # fallback to both if not specified
        ...
        if show_ncr or show_both:
            ...
            results.append(("NCR RECORDS", result_ncr))
        ...
        if show_fcd or show_both:
            ...
            results.append(("FCD RECORDS", result_fcd))
